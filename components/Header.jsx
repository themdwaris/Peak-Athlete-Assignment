  ...
  return (
    <div className="w-full fixed top-0 py-3 px-5 flex items-center justify-between z-50 border-b border-gray-200 bg-linear-to-b from-[#EDEBE4] to-white">
      <div className="flex items-center gap-2">
        ...
        <Link href={"/"} className="cursor-pointer font-medium text-xl md:text-2xl">
          <span>Peak </span>
      ...
      {isLoggedIn ? (
        ...
        <Link href={"/login"} className="px-4 py-1.5 rounded-full cursor-pointer flex items-center justify-center font-semibold md:font-medium gap-1.5 bg-[#dba456] text-sm text-white transition transform active:scale-90 hover:bg-[#c48b3a]">
          Login as Coach
        </Link>
      )}
    </div>
